...
public class BackgroundActionsService extends Service {
	...
	public void onDestroy() {
		...
		tinydb.putListObjectActions("RunningActions",
				new ArrayList<ActionsStruct>());
	...
	public int onStartCommand(Intent intent, int flags, int startid) {
		...
		mBuilder = new NotificationCompat.Builder(getApplicationContext())
				.setSmallIcon(R.drawable.imonmyway_app_launcher_no_background)
		...
		timer.scheduleAtFixedRate(new TimerTask() {
			...
			public void run() {
				remoteViews.setTextViewText(R.id.from,
						"Debug " + Integer.toString(a));
				remoteViews.setTextViewText(R.id.mainText,
						getResources().getString(R.string.running_performing_actions_1) + "\n" + number_of_actions_done
								+ "/" + actionsRunningList.size()
								+ " " + getResources().getString(R.string.running_performing_actions_2));
				remoteViews.setTextViewText(R.id.to, "Running Until: "
						+ endTime);
				mNotificationManager.notify(100, mBuilder.build());
				a++;
				ProcessSecond();
	...
	private void ProcessSecond() {
		...
		if (all_actions_done)
			return; // All Actions Done. Waiting for the user to stop the
					// service...
		...
		for (int i = 0; i < location_list.size(); i++) {
			...
			if (mLastLocation != null) {
				...
				if ((actionsRunningList.get(i).getUsedState() == 0)
						&& (mLastLocation.distanceTo(location_list.get(i)) < 100)) { // 100
																						// meters

					Contact phone = actionsRunningList.get(i).getContactList()
							.get(0);
					if (actionsRunningList.get(i).getType() == 0) {
						String message = actionsRunningList.get(i).getText();
						sendSMS(Integer.toString(phone.getNumber()).trim(),
								message);
						...
						intent.setData(Uri.parse("tel:"
								+ Integer.toString(phone.getNumber()).trim()));
					...
					remoteViews
							.setProgressBar(
									R.id.progress,
									100,
									(int) ((float) number_of_actions_done / (float) actionsRunningList
											.size()) * 100, false);
