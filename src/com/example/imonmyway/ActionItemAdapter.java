    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        try {
        	...
        	if(this.mainList.get(position).getType() == 0){
        		imageResource = this.context.getResources().getIdentifier("@drawable/" + "ic_action_chat", null, this.context.getPackageName());
        		firstLine = "Send a Message to ";
        	}
        	else{
        		imageResource = this.context.getResources().getIdentifier("@drawable/" + "ic_action_call", null, this.context.getPackageName());
        		firstLine = "Make a Call to ";
        	}
        	
        	Drawable res = this.context.getResources().getDrawable(imageResource);
        	mainImage.setImageDrawable(res);
        	
        	List<Contact> contacts = this.mainList.get(position).getContactList();
        	
        	for(int i = 0; i < this.mainList.get(position).getContactList().size(); i++){
        		firstLine += contacts.get(i).getName() + " ";
        	}
        	
            tv2.setText(firstLine + "\nLocation:" + this.mainList.get(position).getLocation() + "\nText: " + this.mainList.get(position).getText());
                        ...
                        popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                            ...
                            public boolean onMenuItemClick(MenuItem item) {
                                ...
                                case R.id.install:
                                    ...
                                    Toast.makeText(context.getApplicationContext(), " Teste " + " : " + position, Toast.LENGTH_LONG).show();

                                    break;
                                case R.id.addtowishlist:
                    
                                	alert_del_location.setMessage("Delete this action?");
